...
@section('content')
    ...
    <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">✏️ Sửa Trạng Thái</h2>
        ...
        <form action="{{ route('admin.statuses.update', $status) }}" method="POST"
            ...
            <div>
                ...
                <input type="text" name="name" id="name" required value="{{ old('name', $status->name) }}"
            ...
            <div>
                <label for="code" class="block text-sm font-medium text-gray-700 mb-1">Mã trạng thái <span
                        class="text-red-500">*</span></label>
                <input type="text" name="code" id="code" required value="{{ old('code', $status->code) }}"
                    class="w-full border border-gray-300 rounded-md px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500">
            </div>
            ...
            <div>
                <label for="type" class="block text-sm font-medium text-gray-700 mb-1">Loại trạng thái <span
                        class="text-red-500">*</span></label>
                <input type="text" name="type" id="type" required value="{{ old('type', $status->type) }}"
                    class="w-full border border-gray-300 rounded-md px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500">
            </div>
            ...
            <div>
                ...
                <input type="color" name="color" id="color" value="{{ old('color', $status->color) }}"
            ...
            <div>
                ...
                <input type="number" name="priority" id="priority" value="{{ old('priority', $status->priority) }}"
                    class="w-full border border-gray-300 rounded-md px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500">
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" id="description" rows="4"
                    class="w-full border border-gray-300 rounded-md px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500"
                    placeholder="Ghi chú thêm về trạng thái...">{{ old('description', $status->description) }}</textarea>
            </div>
