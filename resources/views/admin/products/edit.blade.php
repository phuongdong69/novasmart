      ...
      <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" class="p-6">
          ...
          <div class="mb-4">
            ...
            <input type="text" name="name" id="name" class="border border-gray-300 rounded-md shadow-sm w-full px-3 py-2 mt-0 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Nhập tên sản phẩm" value="{{ old('name', $product->name) }}" required>
            @error('name')
          ...
          <div class="mb-4">
            ...
            <textarea name="description" id="description" class="border border-gray-300 rounded-md shadow-sm w-full px-3 py-2 mt-0 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Mô tả chi tiết sản phẩm">{{ old('description', $product->description) }}</textarea>
            @error('description')
              <p class="text-red-600 text-sm mt-1 mb-1 font-semibold">{{ $message }}</p>
            @enderror
          </div>
          ...
          <div class="mb-4">
            ...
            <select name="origin_id" id="origin_id" class="border border-gray-300 rounded-md shadow-sm w-full px-3 py-2 mt-0 focus:outline-none focus:ring-2 focus:ring-blue-500">
              @foreach ($origins as $origin)
                <option value="{{ $origin->id }}" {{ old('origin_id', $product->origin_id) == $origin->id ? 'selected' : '' }}>{{ $origin->country }}</option>
              @endforeach
            </select>
            @error('origin_id')
              <p class="text-red-600 text-sm mt-1 mb-1 font-semibold">{{ $message }}</p>
          ...
          <!-- Nút Cập nhật -->
          <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded mt-3">Cập nhật sản phẩm</button>
