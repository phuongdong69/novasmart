            ...
            <table class="w-full min-w-0 table-auto divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase">Trạng thái thanh toán
                        </th>
                    ...
                    @forelse($orders as $order)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 text-center" style="position:relative;">
                                ...
                                @endphp
                                <span
                                    style="padding: 6px 12px; background-color: {{ $currentStatus->color ?? '#6b7280' }}; color: white; border-radius: 4px; font-size: 12px; font-weight: 500; display: inline-block; min-width: 80px;">
                                ...
                                @if (!$isCompleted && !$isDelivered && $nextStatus && !($currentStatus && $currentStatus->code === 'cancelled'))
                                    <form action="{{ route('admin.orders.update_status', $order->id) }}" method="POST"
                                        style="display:inline-block; position:absolute; right:8px; top:50%; transform:translateY(-50%);">
                                        @csrf
                            ...
                            <td class="px-4 py-2 flex flex-wrap gap-2 justify-center">
                                ...
                                @php
                                    $cancelStatus = \App\Models\Status::where('type', 'order')
                                        ...
                                        ->first();
                                    $allowCancel =
                                        $currentStatus && in_array($currentStatus->code, ['pending', 'confirmed']);
                                @endphp
                                @if ($cancelStatus)
                                    <form action="{{ route('admin.orders.update_status', $order->id) }}" method="POST"
                                        onsubmit="return {{ $allowCancel ? 'confirm(\'Bạn có chắc chắn muốn hủy đơn hàng này?\')' : 'false' }}">
                                        @csrf
