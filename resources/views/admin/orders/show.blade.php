...
@section('content')
    <div class="w-full px-6 py-6 mx-auto">
            ...
            <div class="flex items-center gap-2">
                ...
                @if ($cancelStatus)
                    <button type="button"
                        ...
                        {{ $allowCancel ? '' : 'disabled' }} title="Hủy đơn hàng">
            ...
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <div class="grid grid-cols-1 gap-0">
                            ...
                            @if (!$isCompleted && !$isDelivered && $nextStatus && !($currentStatus && $currentStatus->code === 'cancelled'))
                                <form action="{{ route('admin.orders.update_status', $order->id) }}" method="POST"
                                    ...
                                    <button type="submit"
                                        ...
                                        onmouseover="this.style.background='linear-gradient(90deg, #7c3aed 0%, #6366f1 100%)'"
                                        onmouseout="this.style.background='linear-gradient(90deg, #6366f1 0%, #06b6d4 100%)'"
                    ...
                    <div class="flex items-center py-2 border-b last:border-b-0">
                        ...
                        <span
                            class="w-3/5 text-lg font-bold text-green-600 text-left">{{ number_format($order->total_price, 0, ',', '.') }}
                            ₫</span>
                    </div>
                    <div class="flex items-center py-2 border-b last:border-b-0">
                ...
                <div class="grid grid-cols-1 gap-0">
                    ...
                    @if ($order->user)
                        <div class="flex items-center py-2 border-b last:border-b-0">
                            ...
                            <span class="w-3/5 text-gray-900 text-left">{{ $order->user->name }}
                    ...
                    <table class="w-full min-w-0 table-auto divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Biến thể (SKU)
                                </th>
                                ...
                                <tr>
                                    <td class="px-6 py-4 align-middle">
                                        <div class="flex items-center">
                                            @php
                                                $thumb = $detail->productVariant->product->thumbnails
                                                    ->where('is_primary', true)
                                                    ->first();
                                            @endphp
                                            @if ($thumb)
                                                <img src="{{ asset('storage/' . $thumb->url) }}"
                                                    alt="{{ $detail->productVariant->product->name }}"
                                                    class="w-12 h-12 object-cover rounded mr-3">
                                            @endif
                                            <div>
                                                <div class="font-medium text-base">
                                                    {{ $detail->productVariant->product->name }}</div>
                                            </div>
                                        </div>
                                    </td>
                                        ...
                                        @if (isset($variant->variantAttributeValues) && count($variant->variantAttributeValues))
                                            <div class="flex flex-col gap-1">
                                                @foreach ($variant->variantAttributeValues as $attrValue)
                                                    <span
                                                        class="inline-block px-2.5 py-0.5 rounded text-base font-semibold bg-blue-100 text-blue-800">
                                                        {{ $attrValue->attributeValue->attribute->name ?? '' }}: <span
                                                            class="font-normal">{{ $attrValue->attributeValue->value ?? '' }}</span>
                                                    </span>
                                                @endforeach
                                            </div>
                                        @else
                ...
                <div class="grid grid-cols-1 gap-0">
                    ...
                    <div class="flex items-center py-2 border-b last:border-b-0">
                        ...
                        <span class="w-3/5 text-left">
                            ...
                            @else
                                <span class="px-2 py-1 rounded text-xs font-semibold bg-gray-300 text-gray-700">Chưa
                                    có</span>
                            @endif
                        </span>
                    </div>
        ...
        <div id="statusLogModal"
            style="display:none; position:fixed; top:20%; left:30%; background:#fff; border:2px solid #333; z-index:9999; padding:40px; box-shadow:0 4px 32px #0002; border-radius:10px; min-width:300px;">
            <button onclick="hideOrderStatusLogModalUnique()"
                style="position:absolute; top:10px; right:10px; background:#eee; border:none; border-radius:4px; padding:4px 10px;">Đóng</button>
                        ...
                        <tbody>
                            @foreach ($statusLogs as $log)
                                <tr>
                                    ...
                                    <td style="border:1px solid #ccc; padding:6px;">
                                        <span
                                            style="background:{{ $log->status->color ?? '#6b7280' }};color:#fff;padding:2px 8px;border-radius:4px">
                                            {{ $log->status->name ?? 'Không xác định' }}
                                        </span>
                                    </td>
                                    <td style="border:1px solid #ccc; padding:6px;">
                                        {{ optional($log->user)->name ?? 'Hệ thống' }}</td>
