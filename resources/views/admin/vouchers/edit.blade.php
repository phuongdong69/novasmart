        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="type" id="type" class="w-full border rounded px-3 py-2" required>
                    ...
                    <option value="fixed" {{ $voucher->type == 'fixed' ? 'selected' : '' }}>Cố định (VNĐ)</option>
                </select>
            </div>

            <div>
                <label for="value" class="block font-medium mb-1">Giá trị giảm *</label>
                <input type="number" name="value" id="value" value="{{ $voucher->value }}" class="w-full border rounded px-3 py-2" required min="0" step="0.01">
            </div>

            <div>
                <label for="max_discount" class="block font-medium mb-1">Mức giảm tối đa (VNĐ)</label>
                <input type="number" name="max_discount" id="max_discount" value="{{ $voucher->max_discount ?? '' }}" class="w-full border rounded px-3 py-2" min="0" step="0.01">
            ...
            <div>
                ...
                <input type="number" name="min_order_value" id="min_order_value" value="{{ $voucher->min_order_value }}" class="w-full border rounded px-3 py-2" required min="0" step="0.01">
            </div>

            <div>
                <label for="usage_limit" class="block font-medium mb-1">Số lần sử dụng tối đa *</label>
                <input type="number" name="usage_limit" id="usage_limit" value="{{ $voucher->usage_limit }}" class="w-full border rounded px-3 py-2" required min="1">
            </div>

            <div>
                <label for="start_date" class="block font-medium mb-1">Ngày bắt đầu *</label>
                <input type="date" name="start_date" id="start_date" value="{{ $voucher->start_date->format('Y-m-d') }}" class="w-full border rounded px-3 py-2" required>
            </div>

            <div>
                <label for="end_date" class="block font-medium mb-1">Ngày kết thúc *</label>
                <input type="date" name="end_date" id="end_date" value="{{ $voucher->end_date->format('Y-m-d') }}" class="w-full border rounded px-3 py-2" required>
            </div>
