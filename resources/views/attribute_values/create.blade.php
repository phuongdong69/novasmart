...
<div class="w-full px-6 py-6 mx-auto">
  ...
  <form action="{{ route('admin.attribute-values.store') }}" method="POST" class="p-6">
    ...
    <div class="mb-4">
      ...
      <input type="text" name="value" id="value" class="border border-gray-300 rounded-md shadow-sm w-full px-3 py-2 mt-0 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Nhập giá trị" value="{{ old('value') }}">
      @error('value')
        <p class="text-red-600 text-sm mt-1 mb-1 font-semibold">{{ $message }}</p>
      @enderror
    </div>

    <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded">Lưu giá trị thuộc tính</button>
  </form>
</div>
@endsection
