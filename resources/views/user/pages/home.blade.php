            ...
            <div class="relative overflow-hidden shadow-sm dark:shadow-gray-800 group-hover:shadow-lg group-hover:dark:shadow-gray-800 rounded-md duration-500">
                ...
                <div class="absolute -bottom-20 group-hover:bottom-3 start-3 end-3 duration-500">
                    @if($variant)
                        ...
                        @endif
                    @else
                        <span class="py-2 px-5 inline-block font-semibold tracking-wide align-middle duration-500 text-base text-center bg-gray-300 text-gray-700 w-full rounded-md cursor-not-allowed">
                            Liên hệ
                        </span>
                    @endif
                </div>
                ...
                @if($variant && $variant->compare_price && $variant->compare_price > $variant->price)
                    <ul class="list-none absolute top-[10px] start-4">
                        <li><span class="bg-orange-600 text-white text-[10px] font-bold px-2.5 py-0.5 rounded h-5">-{{ round(100 - ($variant->price / $variant->compare_price * 100)) }}% Giảm</span></li>
                    </ul>
                @endif
            </div>
