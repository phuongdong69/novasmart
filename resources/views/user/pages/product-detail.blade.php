        ...
        <div class="container relative">
            ...
            <div class="relative mt-3">
                <ul class="tracking-[0.5px] mb-0 inline-block">
                    ...
                    <li class="inline-block uppercase text-[13px] font-bold duration-500 ease-in-out hover:text-orange-500">
                        <a href="{{ route('products.list') }}">Cửa Hàng</a>
                    </li>
                    <li class="inline-block text-base text-slate-950 dark:text-white mx-0.5"><i
                            class="mdi mdi-chevron-right"></i></li>
            ...
            <div class="grid lg:grid-cols-12 md:grid-cols-2 grid-cols-1 gap-6">
                ...
                <div class="lg:col-span-7">
                    <div class="lg:ms-6 sticky top-20">
                        ...
                        <div class="mt-2">
                            <span class="text-red-600 font-semibold me-1"
                                style="color: #dc2626 !important;">{{ number_format($product->price) }} VNĐ</span>
                            ...
                            <ul class="list-none inline-block text-orange-400">
                                @for ($i = 1; $i <= 5; $i++)
                                    <li class="inline">
                                        <i
                                            class="mdi mdi-star{{ $i <= round($averageRating) ? '' : '-outline' }} text-lg"></i>
                                    </li>
                                @endfor
                        ...
                        @if ($product && !$isOutOfStock)
                            ...
                            @if ($relatedVariants->count() > 0)
                                <div class="mt-4 mb-4">
                                    <h5 class="text-lg font-semibold mb-3 text-slate-900 dark:text-white">Các biến thể khác:
                                    </h5>
                                        ...
                                        @foreach ($relatedVariants as $variant)
                                            @php
                                                ...
                                                $imageUrl = $thumbnail
                                                    ...
                                                    : asset('assets/user/images/shop/mens-jecket.jpg');
                                            @endphp
                                            ...
                                            <a href="javascript:void(0)" data-variant-id="{{ $variant->id }}"
                                                ...
                                                <!-- Tên biến thể -->
                                                <div class="text-xs font-medium text-center text-gray-800 mb-1">
                                                    {{ $variantName }}</div>

                                                <!-- Giá -->
                                                <div class="text-xs text-red-600 font-semibold text-center">
                    ...
                    <div id="myTabContent"
                                        ...
                                        @foreach ($productAttributes as $attribute)
                                            <tr
                                                ...
                                                <td class="text-slate-400 pt-4">
                                                    @php
                                                        $isColorAttribute =
                                                            str_contains(strtolower($attribute['name']), 'màu') ||
                                                    ...
                                                    @elseif($isSizeAttribute)
                                                        <span
                                                            class="inline-block px-2 py-1 bg-blue-100 text-blue-800 rounded-md font-medium text-sm">
                        ...
                        <div id="review" role="tabpanel" aria-labelledby="review-tab">
                            ...
                            @php
                                ...
                                $currentStar =
                                    in_array($starFilter, ['1', '2', '3', '4', '5']) && !$isReload
                                        ? $starFilter
                                        : 'all';
                                    ...
                                    <div class="w-1/3 text-center">
                                        <div id="average-rating" class="text-5xl font-bold text-gray-900">
                                            {{ number_format($averageRating, 1) }} <span
                                                class="text-xl text-gray-500">/5</span>
                                        </div>

                                        <div id="average-rating-icons"
                                            class="flex justify-center text-yellow-400 text-xl mt-1">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <i
                                                    class="mdi mdi-star{{ $i <= round($averageRating) ? '' : '-outline' }}"></i>
                                            @endfor
                                        </div>
                                    ...
                                    <div class="flex-1 space-y-2">
                                        @foreach ([5, 4, 3, 2, 1] as $star)
                                            ...
                                            <div id="star-bar-icon"class="flex items-center gap-2">
                                                ...
                                                <div class="flex-1 bg-gray-200 rounded-full overflow-hidden relative"
                                                    style="height: 8px;">
                                                    <div class="bg-red-600 rounded-full rating-bar"
                                                        data-star="{{ $star }}"
                                                        style="width: {{ $percent }}%; height: 100%;"></div>
                                                </div>

                                                <span class="w-20 text-sm text-gray-500 text-right rating-count"
                                                    data-star="{{ $star }}">
                                                    {{ $count }} đánh giá
                                                </span>
                                            </div>
                            ...
                            <div class="bg-white rounded-md p-4 shadow border mb-4">
                                ...
                                <div class="flex flex-wrap gap-2" id="review-filter-buttons">
                                    @foreach (['all' => 'Tất cả', 5 => '5 sao', 4 => '4 sao', 3 => '3 sao', 2 => '2 sao', 1 => '1 sao'] as $value => $label)
                                        <button type="button" data-star="{{ $value }}"
                ...
                {{ $currentStar == $value ? 'bg-blue-100 text-blue-600 border-blue-600' : 'hover:bg-gray-100 text-gray-700' }}">
                                                    ...
                                                    <div class="flex-1">
                                                        <div class="flex items-center gap-2 flex-wrap mb-1">
                                                            ...
                                                            <div class="flex items-center text-yellow-500 text-sm">
                                                                @for ($i = 1; $i <= 5; $i++)
                                                                    <i
                                                                        class="mdi mdi-star{{ $i <= $rating->rating ? '' : '-outline' }}"></i>
                                                                @endfor
                                                            </div>
                                                        ...
                                                        @if ($comment && $comment->content)
                                                            <p class="text-sm text-gray-800 mt-2">{{ $comment->content }}
                                                            </p>
                                                        @endif
                            ...
                            <template id="review-item-template">
                                <div class="py-4 review-item">
                                    <div class="flex items-start gap-4">
                                        <div
                                            class="user-avatar w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold bg-indigo-900 text-white">
        ...
        <section class="relative md:py-24 py-16 bg-gray-50 dark:bg-slate-800">
            <div class="container relative">
                <div class="grid grid-cols-1 mb-6 text-center">
                    ...
                    <p class="text-slate-400 mt-3">Các sản phẩm cùng thương hiệu
                        {{ $product->product->brand->name ?? 'N/A' }}</p>
                </div>
